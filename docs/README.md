...
- [x]  이때, 아래와 같은 경우  다시 `Randoms.pickNumberInRange()`를 통해 임의의 값을 생성해서 추천할 카테고리를 정해야 한다.
    - [x]  한 주에 중복된 카테고리가 2회를 초과하는 경우
2. 추천된 카테고리를 저장한다.
...
- [x]  이때, 아래와 같은 경우 다시 섞은 후 첫 번째 값을 사용해야 한다.
    - [x]  한 주에 중복되는 메뉴가 있을 경우
    - [x]  해당 코치가 못 먹는 메뉴에 포함될 경우
3. 추천된 점심 메뉴를 저장한다.
